...
for (ni in 1:length(Ns)) {
    ...
    # dev.new()
    g = ggplot()
    # g = g + geom_point(aes(x=loosums, y=tls))
    g = g + geom_bin2d(aes(x=loosums, y=tls), bins=40, show.legend=FALSE)
    g = g + scale_fill_gradient(low="gray", high="black")
    g = g + geom_abline(intercept=0, slope=1, colour='orangered1')
    if (MEASURE >= 4) {
        g = g + geom_hline(yintercept=0, colour='orangered1')
        g = g + geom_vline(xintercept=0, colour='orangered1')
    }
