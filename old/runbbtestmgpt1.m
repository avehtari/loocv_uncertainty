...
%   Last modified: 2016-10-25 11:45:30 EDT

clear
N=[10 20 30 50 70 100];
gp=gp_set('cf',gpcf_constant('constSigma2',1,'constSigma2_prior',[]));
...
for ni=1:numel(N)
    ...
    clear loos pks ltrs
    for i1=1:1000
        gpia=gp_ia(gp,x,y(i1,:)','int_method','grid','display','off');
        [~,~,ltst00]=gp_pred(gpia,x,y(i1,:)',xtt,'yt',ytt);
        ltst0(i1,:)=mean(ltst00)*n;
        [~,~,ltrs(i1,:)]=gp_pred(gpia,x,y(i1,:)');
        [~,~,loos(i1,:)]=gp_loopred(gpia,x,y(i1,:)');
    end
