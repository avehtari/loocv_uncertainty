...
# beta effect for model B, missing in model A
beta_B_s = np.array([0.0, 0.5, 1.0, 2.0])
...
beta_s = np.zeros((len(beta_B_s), n_dim))
for b_i, beta_B in enumerate(beta_B_s):
...
if load_res:
    ...
    mean_loo_s = np.full((len(beta_B_s), len(n_obs_s), n_trial), np.nan)
    var_loo_s = np.full((len(beta_B_s), len(n_obs_s), n_trial), np.nan)
    skew_loo_s = np.full((len(beta_B_s), len(n_obs_s), n_trial), np.nan)
    mean_err_s = np.full((len(beta_B_s), len(n_obs_s), n_trial), np.nan)
    var_err_s = np.full((len(beta_B_s), len(n_obs_s), n_trial), np.nan)
    skew_err_s = np.full((len(beta_B_s), len(n_obs_s), n_trial), np.nan)
    ...
    for i0, beta in enumerate(beta_s):
        ...
        print('{}/{}, elapsed time: {:.2} min'.format(
            i0+1, len(beta_B_s), cur_time_min), flush=True)
