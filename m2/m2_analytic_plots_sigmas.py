...
if load_res:
    ...
    # variables
    sigma2_d_s = np.linspace(1.0, 100.0, 5)
    sigma2_d_s = [1.0, 5.0, 10.0, 50.0, 100.0, 500.0, 1000.0]
    ...
    analytic_mean_s = np.full(
        (len(prc_out_s), len(sigma2_d_s), len(n_obs_s), n_trial), np.nan)
    ...
    analytic_coefvar_s = np.full(
        (len(prc_out_s), len(sigma2_d_s), len(n_obs_s), n_trial), np.nan)
    for i3, sigma2_d in enumerate(sigma2_d_s):
        cur_time_min = (time.time() - start_time)/60
        print('{}/{}, elapsed time: {:.2} min'.format(
            i3+1, len(sigma2_d_s), cur_time_min), flush=True)
    ...
    for n_i, ax in enumerate(axes):
        for o_i, prc_out in enumerate(prc_out_s):
            ...
            if plot_multilines:
                ...
                ax.fill_between(sigma2_d_s, q025, q975, alpha=0.2)
                ax.plot(sigma2_d_s, median)
