...
qpp = array(NaN, c(Niter, length(alphas), length(Ns)))

quantiles = c(0.05, 0.10, 0.90, 0.95)
viol_grid_rmsebb = 50
accs_q_rmsebb = array(NaN, c(length(alphas), length(Ns), length(quantiles), 5))
accs_m_rmsebb = array(NaN, c(length(alphas), length(Ns), length(quantiles)))
accs_viol_rmsebb = array(
    NaN, c(length(alphas), length(Ns), length(quantiles), 2, viol_grid_rmsebb))
...
for (ni in 1:length(Ns)) {
    ...
    # generate y
    if (truedist=="b") {
        ys = matrix(NaN, 2*n, Niter)
    } else {
        ys = matrix(NaN, n, Niter)
    }
    for (i1 in 1:Niter) {
        set.seed(i1)
        if (truedist=="n") {
            ys[,i1] = as.array(rnorm(n))
        } else if (truedist=="t4") {
            ys[,i1] = as.array(rt(n,4))
        } else if (truedist=="b") {
            ys[,i1] =  as.array(as.double(kronecker(matrix(1,1,n),t(c(0,1)))))
        } else {
    ...
    for (alpha_i in 1:length(alphas)) {
        ...
        if (truedist=="b") {
            dw = rdirichlet(bbsamples, rep(alpha, 2*n))
        } else {
            dw = rdirichlet(bbsamples, rep(alpha, n))
        }
        qw = 1 - t(t(dw%*%((ys-muloos)^2))/colVars(ys))
        qpp[,alpha_i,ni] = colMeans(qw > 0)
    ...
    for (alpha_i in 1:length(alphas)) {
        ...
        for (qi in 1:length(quantiles)) {
            ...
            b_alpha = c+1
            b_beta = Niter-c+1
            accs_m_rmsebb[alpha_i,ni,qi] = b_alpha / (b_alpha + b_beta)
            accs_q_rmsebb[alpha_i,ni,qi,] = qbeta(
                c(0.025, 0.25, 0.5, 0.75, 0.975), b_alpha, b_beta)
            accs_viol_rmsebb[alpha_i,ni,qi,1,] = seq(
                qbeta(0.01, b_alpha, b_beta),
                qbeta(0.99, b_alpha, b_beta),
                length=viol_grid_rmsebb)
            accs_viol_rmsebb[alpha_i,ni,qi,2,] = dbeta(
                accs_viol_rmsebb[alpha_i,ni,qi,1,], b_alpha, b_beta)
...
for (qi in 1:length(quantiles)) {
    g = plot_known_viol(
        Ns, accs_viol_rmsebb[alpha_i,,qi,1,], accs_viol_rmsebb[alpha_i,,qi,2,],
        # range1 = accs_q_rmsebb[alpha_i,,qi,c(1,5)],  # 95 % interval
        # range2 = accs_q_rmsebb[alpha_i,,qi,c(2,4)],  # 50 % interval
        line = accs_q_rmsebb[alpha_i,,qi,3],  # median
        # point = accs_m_rmsebb[alpha_i,,qi],  # mean
        colors='blue'
    )
    g = g +
