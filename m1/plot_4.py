...
for o_i, o_ii in enumerate(selected_prc_outs):
    ...
    fig, axes = plt.subplots(
        len(selected_ns), len(selected_betas), figsize=(6,6),
        sharex='col',
        sharey=False
    )
    ...
    for n_i, n_ii in enumerate(selected_ns):
        ...
        for b_i, b_ii in enumerate(selected_betas):
            ...
            data_x = loo_s[o_i, b_i, n_i]
            data_y = np.sqrt(naive_var_s[o_i, b_i, n_i])
            data_z = bma_s[o_i, b_i, n_i, :, 0]

            # select a sparce representation
            idxs = select_grid_subset(data_x, data_y, 30, 30)

            scat = ax.scatter(
                data_x[idxs], data_y[idxs], s=8, c=data_z[idxs],
            ...
            if x_0 < 0:
                ax.plot(
                    [x_0, 0], [-2*x_0, 0],
                    label='2SE', color='C2', zorder=1, lw=0.8
                )
                ax.plot(
                    [x_0, 0], [-3*x_0, 0],
                    label='3SE', color='C3', zorder=1, lw=0.8
                )
            if x_1 > 0:
                ax.plot(
                    [0, x_1], [0, 2*x_1],
...
for o_i, o_ii in enumerate(selected_prc_outs):
    ...
    for n_i, n_ii in enumerate(selected_ns):
        ...
        for b_i, b_ii in enumerate(selected_betas):
            ...
            data_x = loo_s[o_i, b_i, n_i]
            # data_y = (1-naive_plooneg_s[o_i, b_i, n_i]) - bma_s[o_i, b_i, n_i, :, 0]
            data_y = bma_s[o_i, b_i, n_i, :, 0] - bma_elpd_s[o_i, b_i, n_i, :, 0]

            ax.hexbin(data_x, data_y, gridsize=20, cmap=cmap, mincnt=1)
    ...
    for ax in axes[-1,:]:
        ax.set_xlabel(r'$\mathrm{\widehat{elpd}_d}$', fontsize=16)

    for ax in axes[:,0]:
        ax.set_ylabel(r'$w_\mathrm{estim} - w_\mathrm{true}$',fontsize=16)
...
for o_i, o_ii in enumerate(selected_prc_outs):
    ...
    fig, axes = plt.subplots(
        len(selected_ns), len(selected_betas), figsize=(8,9),
        sharex='col',
        sharey=True
    )
    ...
    for n_i, n_ii in enumerate(selected_ns):
        ...
        for b_i, b_ii in enumerate(selected_betas):
            ...
            data_x = loo_s[o_i, b_i, n_i]
            # data_x = loo_s[o_i, b_i, n_i] / np.sqrt(naive_var_s[o_i, b_i, n_i])

            # data_y = (1-naive_plooneg_s[o_i, b_i, n_i]) - bma_s[o_i, b_i, n_i, :, 0]
            # data_y = bma_s[o_i, b_i, n_i, :, 0] - bma_elpd_s[o_i, b_i, n_i, :, 0]
            data_y = bma_s[o_i, b_i, n_i, :, 0]

            data_z = bma_s[o_i, b_i, n_i, :, 0] - bma_elpd_s[o_i, b_i, n_i, :, 0]

            # select a sparce representation
            idxs = select_grid_subset(data_x, data_y, 40, 40)
    ...
    cbar.set_label('pBMA+ error', rotation=90, fontsize=16)
