...
gs_s = np.array([
    [
        gridspec.GridSpecFromSubplotSpec(
            len(out_dev_sel), len(datas),
            subplot_spec=outer[gs_i, gs_j], hspace=0.1, wspace=0.26,
        )
...
axes = np.array([
    [
        [
            [
                fig.add_subplot(gs_s[gs_i, gs_j][o_i, d_i])
                for d_i in range(len(datas))
            ]
            for o_i in range(len(out_dev_sel))
        ]
        for gs_j in range(len(n_obs_sel))
    ]
    for gs_i in range(len(beta_t_sel))
    ...
    for beta_t_i, beta_t in enumerate(beta_t_sel):
        for o_i, out_dev in enumerate(out_dev_sel):
            ...
            for d_i in range(len(datas)):
                ...
                data = datas[d_i][probl_i]
                ax = axes[beta_t_i, n_obs_i, o_i, d_i]
                ax.bar(
                    cal_limits[:-1],
                    data,
                    width=1/cal_nbins,
            ...
            maxy = np.max([
                ax.get_ylim()[1]
                for ax in axes[beta_t_i, n_obs_i, o_i, :]
            ])
            for ax in axes[beta_t_i, n_obs_i, o_i, :]:
                ax.set_ylim(0, maxy)
...
for n_obs_i, n_obs in enumerate(n_obs_sel):
    for beta_t_i, beta_t in enumerate(beta_t_sel):
        for o_i, out_dev in enumerate(out_dev_sel):
            for d_i in range(len(datas)):
                ax = axes[beta_t_i, n_obs_i, o_i, d_i]
                if not (beta_t_i==len(beta_t_sel)-1 and o_i==len(out_dev_sel)-1):
                    ax.set_xticklabels([])
                else:
...
# set out labels
for o_i, out_dev in enumerate(out_dev_sel):
    for ax in axes[:, 0, o_i, 0]:
        ax.set_ylabel(
            'outlier' if o_i else 'no outlier',
            rotation=0,
            ha='right',
            va='center',
            fontsize=fontsize-2,
...
# set data labels
for d_i, d_name in enumerate(d_names):
    for ax in axes[0, :, 0, d_i]:
        ax.set_title(
            d_name,
            ha='right',
            va='center',
...
# set n labels
for ax, n_obs in zip(axes[0, :, 0, 0], n_obs_sel):
