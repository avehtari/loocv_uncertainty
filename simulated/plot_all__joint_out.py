...
for n_obs_i, n_obs in enumerate(n_obs_sel):
    for beta_t_i, beta_t in enumerate(beta_t_sel):
        ...
        #     zoom_limits[beta_t_i, n_obs_i])

        lims = np.percentile(x_arr, [1, 99])
        zoom_xmin, zoom_xmax = lims+np.array([-1,1])*0.25*(lims[1]-lims[0])
        lims = np.percentile(y_arr, [1, 99])
        zoom_ymin, zoom_ymax = lims+np.array([-1,1])*0.25*(lims[1]-lims[0])
        # lims = np.percentile(x_arr, [0, 100])
