...
n_probl = len(beta_t_s)
n_trial = None
for probl_i, beta_t in enumerate(beta_t_s):
...
plt.figure()
plt.plot(beta_t_s, np.zeros(n_probl), label=None, color='r')
plt.plot(beta_t_s, loo_y_mean_p, label='loo_y_mean')
plt.plot(beta_t_s, elpd_y_mean_p, label='elpd_y_mean')
plt.xlabel('beta_t')
