...
sd2hat_mean_bb_pb, sd2hat_sd_bb_pb, sd2hat_skew_bb_pb = bb_mean_sd_skew(loo_var_hat_pt)

# ratio SE_hat / SE of err(y)
loo_var_p = np.var(loo_pt, ddof=1, axis=-1)
err_var_p = np.var(err_pt, ddof=1, axis=-1)
se2hat_looratio_mean_p = np.sqrt(np.mean(loo_var_hat_pt, axis=-1)/loo_var_p)
se2hat_errratio_mean_p = np.sqrt(np.mean(loo_var_hat_pt, axis=-1)/err_var_p)
bb_rng = np.random.RandomState(seed=54134265)
w_bt = bb_rng.dirichlet(np.full(n_trial, 1.0), size=2000)
se2hat_looratio_pb = np.sqrt(
    np.einsum('bt,pt->pb', w_bt, loo_var_hat_pt)/loo_var_p[:,None])
se2hat_errratio_pb = np.sqrt(
...
datas = [
    [se2hat_looratio_pb, loo_skew_bb_pb],
    [se2hat_errratio_pb, err_skew_bb_pb],
]
data_names = [
    r'$\widehat{\mathrm{elpd}}_\mathrm{LOO}$',
    r'$\mathrm{err}_\mathrm{LOO}$'
...
data_statistic_names = [ylabel1, 'skewness']
show_zero_line = [
    [False, True],
    [False, True],
]
...
for d_j, data_i in enumerate(datas):
    ...
    for d_i, data_ij in enumerate(data_i):
        ...
        for b_i, beta_t in enumerate(beta_t_sel):
            ...
            ax.fill_between(np.arange(len(n_obs_sel)), q025, q975, alpha=0.2, color=color)
            median = np.percentile(data, 50, axis=-1)
            # ax.plot(n_obs_sel, median, color=color, label=label)
            ax.plot(np.arange(len(n_obs_sel)), median, color=color, label=label)

        if show_zero_line[d_j][d_i]:
            ax.axhline(0, color='gray', lw=1.0)#, zorder=0)
        if show_one_line[d_j][d_i]:
            ax.axhline(1, color='gray', lw=1.0)#, zorder=0)

        ax.set_xticks(np.arange(len(n_obs_sel)))
