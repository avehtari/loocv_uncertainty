...
# for probl_i in range(n_probl):
#     n_obs, beta_t, out_dev = probl_i_to_params(probl_i)
#     data_pt[probl_i,:] /= n_obs
# ylabel = (
#     r'${}^\mathrm{sv}\mathrm{err}_\mathrm{LOO}(\mathrm{M}_a,\mathrm{M}_b|y) / n$'
#     # r'$\mathrm{err}_\mathrm{LOO} / n$'
# )

data_pt = err_pt / np.std(err_pt, ddof=1, axis=-1, keepdims=True)
# ylabel = (
#     r'${}^\mathrm{sv}\mathrm{err}_\mathrm{LOO}(\mathrm{M}_a,\mathrm{M}_b|y)'
#     r'/'
#     r'\mathrm{SD}({}^\mathrm{sv}\mathrm{elpd}(\mathrm{M}_a,\mathrm{M}_b|y))$'
# )
ylabel = (
